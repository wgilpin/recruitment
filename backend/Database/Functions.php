...
class localEveDB
{
    ...
    public function __construct()
    {
        ...
        $array = ConfigReturn();
        $this->Host = $array[EVEDBHost];
        $this->dbName = $array[EVEDBdbName];
        $this->dbPass = $array[EVEDBdbPass];
        $this->Charset = $array[EVEDBCharset];
        $this->inventoryType = $array[EVEDBinventoryType];
        $this->stations = $array[EVEDBstations];
...
class localEveCache extends localEveDB
{
    ...
    public function __construct()
    {
        ...
        $array = ConfigReturn();
        $this->daysCharacter = date("Y-m-d H:i:s", strtotime($array[daysCharacter]));
        $this->daysCorporation = date("Y-m-d H:i:s", strtotime($array[daysCorporation]));
        $this->daysAlliance = date("Y-m-d H:i:s", strtotime($array[daysAlliance]));
        $this->daysStructure = date("Y-m-d H:i:s", strtotime($array[daysStructure]));
    ...
    private function structNotFoundFixer($array, $structFunc, $corpFunc, $allyFunc, $unknownFunc)
    {
        $temp2 = array();
        $temp = "";
        ...
        if ($temp) {
            ...
            $allarray = $this->notFoundIDFixer($selectquery, "", $corpFunc, $allyFunc, $unknownFunc);
            if ($allarray["upload"]) {
    ...
    public function structCache($array, $structFunc, $corpFunc, $allyFunc, $unknownFunc)
    {
        ...
        if ($data2["upload"]) {
            $this->insertUpdate($data2["upload"], $this->keyArrayStruct);
        }
...
//    public function __construct()
//    {
//        include '../../Include.php'; _include(array("Config"));
