import React from 'react';
import PropTypes from 'prop-types';
import Alt from './Alt';
import ApplyAlts from './ApplyAlts';
import Tab from './Tab';
import TabButton from './TabButton';
import ApplyQs from './ApplyQs';

const styles = {
  h1: {
    color: "#01799a",
  },
  outer: {
    display: 'flex flex-column',
  },
  alt: {
    marginLeft: "32px",
  },
  para: {
    marginBottom: "24px",
    marginTop: "24px",
  },
  tabs: {

  },
  submit: {
    marginTop: "12px",
    width: "200px",
    height: "32px",
    backgroundColor: "#01799a",
    color: "white",
    fontSize: 16,
    fontWeight: 600,
    borderWidth: "0px",
  },
  submitDisabled: {
    backgroundColor: "#B5D8E2",
  }

};


export default class Apply extends React.Component {
  constructor(props) {
    super(props);
    this.state = { selected: "Alts", canSubmit: true };
  }

  onButtonClick = (e) => {
    console.log(e);
    this.setState({ selected: e })
  }

  render() {
    let buttonStyle = styles.submit;
    if (!this.state.canSubmit){
      buttonStyle = {...styles.submit, ...styles.submitDisabled};
    }
    return (
      <>
        <h1 style={styles.h1}>My Application</h1>
        <div style={styles.alt}>
          <Alt
            name="Will Gilpin"
            src="data:image/jpeg;base64,"
          />
        </div>
        <div style={styles.para}>
          <p>
            So this is how it works. Click "Alts" and add all your alts, then click "Questions" and start answering.
          </p>
          <p>You can come back at any time to continue, until you click "Submit" and then it's locked and a recruiter will get to work</p>
        </div>
        <Tab>
          <TabButton
            label="Alts"
            selected={this.state.selected === "Alts"}
            onClick={this.onButtonClick}
          />
          <TabButton
            label="Questions"
            selected={this.state.selected === "Questions"}
            onClick={this.onButtonClick}
          />
        </Tab>
        <div>
          {this.state.selected === "Alts" && <ApplyAlts />}
          {this.state.selected === "Questions" && <ApplyQs />}
        </div>
        <div>
          <button style={buttonStyle} disabled={true}>Submit</button>
        </div>
      </>
    );
  }
}